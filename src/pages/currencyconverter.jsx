...
const CurrencyConverter = () => {
  ...
  const fetchCurrenciesAndDetails = async () => {
    ...
    try {
      ...
      const detailsResponse = await fetch('https://openexchangerates.org/api/currencies.json');
  ...
  const getCurrencyCountry = (code, name) => {
    ...
    return commonCurrencyCountries[code] || 
           name.includes('Pound') ? name.replace(' Pound', '') :
           name.includes('Dollar') ? name.replace(' Dollar', '') :
           name.includes('Franc') ? name.replace(' Franc', '') :
           name;
  };
